...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: twhutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := twhutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*twhutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, twhutil.NewTx(tx))
	}
...
func additionalCoinbase(amount twhutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee twhutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee twhutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee twhutil.Amount) *wire.MsgTx {
