...
const (
	...
	DefaultMinRelayTxFee = twhutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee twhutil.Amount) int64 {
	...
	if minFee < 0 || minFee > twhutil.MaxSatoshi {
		minFee = twhutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *twhutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func IsDust(txOut *wire.TxOut, minRelayTxFee twhutil.Amount) bool {
...
func checkTransactionStandard(tx *twhutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee twhutil.Amount,
...
func GetTxVirtualSize(tx *twhutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
