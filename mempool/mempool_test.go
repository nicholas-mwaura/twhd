...
func (s *fakeChain) FetchUtxoView(tx *twhutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *twhutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *twhutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   twhutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*twhutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee twhutil.Amount,
	signalsReplacement bool) (*twhutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput twhutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*twhutil.Tx, error) {
	txChain := make([]*twhutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, twhutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := twhutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *twhutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee twhutil.Amount,
	signalsReplacement, confirmed bool) *twhutil.Tx {
...
func testPoolMembership(tc *testContext, tx *twhutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   twhutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *twhutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*twhutil.Tx, []*twhutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*twhutil.Tx, []*twhutil.Tx) {
				...
				return tx, []*twhutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*twhutil.Tx, []*twhutil.Tx) {
				...
				return tx, []*twhutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = twhutil.SatoshiPerBitcoin
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(ctx *testContext) (*twhutil.Tx, []*twhutil.Tx) {
				...
				return childReplace, []*twhutil.Tx{childNoReplace}
			},
			err: "",
		},
	}
