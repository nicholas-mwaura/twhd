...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *twhutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *twhutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *twhutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a twhutil.Address) {
	switch a := a.(type) {
	case *twhutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *twhutil.AddressScriptHash:
		f.scriptHashes[*a.Hash160()] = struct{}{}
		return
	case *twhutil.AddressPubKey:
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := twhutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a twhutil.Address) bool {
	switch a := a.(type) {
	case *twhutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a twhutil.Address) {
	switch a := a.(type) {
	case *twhutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected twhutil.Block
type notificationBlockDisconnected twhutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *twhutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *twhutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *twhutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, twhutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(btcjson.RpcVersion1, nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*twhutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *twhutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *twhutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *twhutil.Tx, block *twhutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *twhutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *twhutil.Tx, block *twhutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := twhutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *twhutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *twhutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *twhutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *btcjson.RescanCmd, lookups *rescanKeys, minBlock,
	...
	*twhutil.Block, *chainhash.Hash, error) {
