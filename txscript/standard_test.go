...
// newAddressPubKey returns a new twhutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) twhutil.Address {
	addr, err := twhutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new twhutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) twhutil.Address {
	addr, err := twhutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new twhutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) twhutil.Address {
	addr, err := twhutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the twhutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the twhutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := twhutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := twhutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := twhutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := twhutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := twhutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*twhutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*twhutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*twhutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*twhutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*twhutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
