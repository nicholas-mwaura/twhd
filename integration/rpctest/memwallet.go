...
import (
	...
	"github.com/nicholas-mwaura/twhutil/hdkeychain"
)
...
type memWallet struct {
	...
	addrs map[uint32]twhutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]twhutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*twhutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          twhutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (twhutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []twhutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt twhutil.Amount,
	feeRate twhutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := twhutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate twhutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate twhutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt twhutil.Amount
	for _, output := range outputs {
		outputAmt += twhutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() twhutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance twhutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (twhutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := twhutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
