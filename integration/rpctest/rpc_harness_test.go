...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt twhutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(twhutil.Amount(5 * twhutil.SatoshiPerBitcoin))
	...
	txid = genSpend(twhutil.Amount(500 * twhutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(twhutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*twhutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, twhutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := twhutil.Amount(250 * twhutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = twhutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := twhutil.Amount(50 * twhutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := twhutil.Amount(numMatureOutputs * 50 * twhutil.SatoshiPerBitcoin)
